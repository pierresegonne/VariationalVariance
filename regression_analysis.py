...
def regression_subplot(alg, prior, ll_logger, data_logger, mv_logger, ax, color):
    ...
    i_best = ll_logger.query("Algorithm == '" + alg
                             + "' and Prior == '" + prior + "'")['LL'].idxmax()

    # plot the training data
    data = data_logger.query("Algorithm == '" + alg
                             + "' and Prior == '" + prior + "'").loc[i_best]
    sns.scatterplot(data['x'], data['y'], ax=ax, color=color)

    # plot the model's mean and standard deviation
    model = mv_logger.query("Algorithm == '" + alg
                            + "' and Prior == '" + prior + "'").loc[i_best]
    ...
    truth = mv_logger.query(
        "Algorithm == 'truth' and Prior == 'N/A'").loc[i_best]
...
def toy_regression_plot(ll_logger, data_logger, mv_logger):
    ...
    fig = plt.figure(figsize=(2.9 * n_cols, 2.9 * n_rows),
    ...
    # make it tight
    plt.subplots_adjust(left=0.05, bottom=0.07, right=0.98,
    ...
    for i, prior in enumerate(priors):
        ...
        regression_subplot(alg1, prior, ll_logger,
                           data_logger, mv_logger, ax, colors[0])
        ax.set_xlim([-5, 15])
        ax.set_ylim([-25, 25])
        ax.set_xlabel('')
        ax.set_xticklabels([])
        ...
        truth = mv_logger.query(
            "Algorithm == 'truth' and Prior == 'N/A'").loc[0]
        ax.plot(truth['x'], truth['std(y|x)'], 'k', label='truth')
        query = "(Algorithm == '" + alg1 + "') and Prior == '" + prior + "'"
        sns.lineplot(x='x', y='std(y|x)', hue='Method', ci='sd',
                     data=mv_logger.query(query), ax=ax)
        ax.legend().remove()
        ax.set_xlim([-5, 15])
        ax.set_ylim([0, 5])
        if i > 0:
...
def uci_regression_analysis():
    ...
    for mode in ['10000', 'full']:
        ...
        if mode == '10000':
            ...
            post_fix = ''
            results = [
                result for result in pickles if '10000' not in result and 'full' not in result]
        with open(os.path.join('assets', 'regression_uci_ll' + post_fix + '.tex'), 'w') as f:
            print(build_table(results, 'LL', 'max', max_cols,
                              post_fix=post_fix, process_fn=process_fn), file=f)
        with open(os.path.join('assets', 'regression_uci_mae' + post_fix + '.tex'), 'w') as f:
            print(build_table(results, 'MAE', 'min', max_cols,
                              post_fix=post_fix, process_fn=process_fn), file=f)
        with open(os.path.join('assets', 'regression_uci_rmse' + post_fix + '.tex'), 'w') as f:
            print(build_table(results, 'RMSE', 'min', max_cols,
                              post_fix=post_fix, process_fn=process_fn), file=f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment', type=str,
                        default='uci', help='experiment in {toy, uci}')
