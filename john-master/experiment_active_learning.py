...
def john(args, X, y, Xpool, ypool, Xtest, ytest):
    ...
    while it < args.iters:
        ...
        if it % its_per_epoch == 0:
            ...
            if it > args.iters * 0.60:
                ...
                with torch.no_grad():
                    v_pool = model(x_pool, switch)[1].mean(dim=0) * y_std ** 2
                    var_list.append(v_pool.cpu().numpy())
                model.train()
    ...
    return ll_list[i_best], mae_list[i_best], rmse_list[i_best], var_list[i_best]
...
def detlefsen_uci_baseline(x_train, y_train, x_pool, y_pool, x_test, y_test, iterations, batch_size):
    ...
    ll, mae, rmse, var = john(args, x_train, y_train, x_pool, y_pool, x_test, y_test)

    return ll, mae, rmse, var
